...
// Derived wire metrics are floating-point; compare with a tolerance appropriate to
// their magnitude rather than requiring exact binary equality.
bool near(double value, double expected) { return std::abs(value - expected) < 1e-9; }
...
GenerationOutcome sample_outcome() {
    ...
    outcome.metrics.ttft_seconds            = 0.12;
    outcome.metrics.decode_seconds          = 0.036;
    return outcome;
}
...
int test_aggregate_response() {
    ...
    // vLLM-style `metrics` object (TTFT/ITL) for routers; 6 decode steps of 0.036s -> 6ms.
    failures += check(near(response["metrics"]["time_to_first_token_ms"].get<double>(), 120.0) &&
                          near(response["metrics"]["mean_itl_ms"].get<double>(), 6.0),
                      "aggregate response carries derived generation metrics");

    GenerationOutcome unmeasured = sample_outcome();
    unmeasured.metrics.ttft_seconds = 0.0;
    failures += check(!Json::parse(make_chat_completion_response(identity(), unmeasured)).contains(
                         "metrics"),
                      "metrics omitted when no first token was produced");
...
int test_stream_response() {
    ...
    failures += check(near(parse_sse(events[1])["metrics"]["time_to_first_token_ms"].get<double>(),
                           120.0) &&
                          near(parse_sse(events[1])["metrics"]["mean_itl_ms"].get<double>(), 6.0),
                      "stream terminal chunk carries derived generation metrics");
