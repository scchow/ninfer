...
void terminalize(DecoderState& state, const StopPolicy& policy, PublishedOutput& emitted,
    ...
    if (state.in_reasoning) {
        ...
        // Whatever is held here is a suffix of the think-close marker (or bytes containing
        // one). No further tokens can resolve it: publish ordinary generated bytes as
        // content but never a close marker itself - terminating on one must match what
        // decoding would have done with it (dropped), not leak it into content.
