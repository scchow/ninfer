...
// (completion - 1) tokens come from decode. Returns an empty object when no
// first token was produced (nothing to time).
Json metrics_extra(const GenerationOutcome& outcome) {
    const GenerationMetrics& metrics = outcome.metrics;
    if (metrics.ttft_seconds <= 0.0) { return Json::object(); }
    ...
    return Json{{"metrics",
                 Json{{"time_to_first_token_ms", metrics.ttft_seconds * 1000.0},
...
std::string chunk(const OpenAIChatResponseIdentity& identity, Json delta, Json finish_reason,
                  bool include_usage, Json extra = Json::object()) {
...
std::string make_chat_completion_response(const OpenAIChatResponseIdentity& identity,
    ...
    const Json extra = metrics_extra(outcome);
    for (auto it = extra.begin(); it != extra.end(); ++it) { payload[it.key()] = it.value(); }
    return payload.dump();
}
...
std::vector<std::string> OpenAIChatStream::finish(const GenerationOutcome& outcome) {
    ...
    if (!outcome.tool_calls.empty()) {
        ...
        events.push_back(
            chunk(identity_, Json::object(), "tool_calls", include_usage_,
                  metrics_extra(outcome)));
    } else {
        events.push_back(chunk(identity_, Json::object(), finish_reason(outcome.finish_reason),
                               include_usage_, metrics_extra(outcome)));
    }
